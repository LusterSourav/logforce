...
- Drop zone `Drop log file here or paste below, live output/multi → POST /api/ingest` with format menu (`Palo Alto`, `Cisco ASA`, `FortiGate`, `CEF`, `LEEF`, `Suricata`, `Zeek`, `syslog`, `JSON`, `CSV`, `XML`, `LogForce OCSF`) → `?format=`.
...
- Settings → Quick Settings → `+` → tiles list shows new tile **LogForce** (shield icon, like Rethink) beside `Refresh Connection`.
- Tap once: tile animates `inactive → active → inactive` (Rethink does same), status bar shows `VPN LogForce` key icon, toast: `LogForce Capture, connected to Azure pod 20.193.x.x`.
- Crash at 2 AM: app shows `Report?` dialog → `Send` → same path, or user taps **LogForce** tile → log auto-attached → toast `OuterTune Error 2000, Auth expired, FIX ready → Copy`.
...
[ Refresh Connection | LogForce ← ours together with Rethink ]
```

**Implementation (Kotlin, minimal):**
```kotlin
// app/src/main/java/com/logforce/capture/LogForceTileService.kt
class LogForceTileService: TileService() {
...
**Non-Android (future):**
- iOS: Shortcuts action `LogForce Capture` + Share Extension `Send to LogForce`; same `POST` via WireGuard App.
- Laptop: tray icon `LogForce` (macOS menu bar / Win system tray) → click → tails `DiagnosticReports`/`EventLog`, same toast.
...
TileService (LogForceTileService.kt, BIND_QUICK_SETTINGS_TILE)
...
Tile has `android:contentDescription="LogForce Capture"`, TalkBack reads hint; chart has text counts; keyboard focus on input.
...
Night. Android: QS → Add tile LogForce beside Rethink (your screenshot)
 5. Trigger crash → tap LogForce → WireGuard → toast "Error 2000, token, FIX Copy curl"
...
**2 Core UX:** Prototype `paste → Classify → 2→3` is real (measured 60 ms/100 lines). Future `any device auto → WireGuard → pod → fix` is not yet fully wired, `auto_capture/server.py` + `PHONE_TILE.md TileService` exist, but no `LogForce.apk` built yet. Design shows intent, not shipped APK.

**3 Main Screens:** Pipeline hero card is shipped (`ui/dashboard.html:1215` exact). QS tile row `LogForce beside Rethink` is a **mock in the design image**, real Rethink screenshot shows Refresh/Rethink/Orbot, LogForce tile will appear there only after APK install + `BIND_QUICK_SETTINGS_TILE`. Not fake, but not installed on your phone today.
