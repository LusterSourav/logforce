...
|---|---|---|
| R01 | `LogForce-Perimeter-Prototype/ui/server.go,136,154,214,290,398,521` | 5 handlers `health/classify/ingest/query/stats` + live `2→3` |
| R02 | `LogForce-Perimeter-Prototype/models/model_quantized.onnx` 215K + `onnx_data` 22M + `libonnxruntime.dylib` 34M = 58M | ONNX 23 MB claim (model only ~22 MB, total 58M) |
...
| R04 | `LogForce-Perimeter-Prototype/ingestion/vector.toml,79` + `transforms/normalize_perimeter.vrl,74` | File tail + VRL 3-phase + Phase 1 plug point |
| R05 | `LogForce-Perimeter-Prototype/storage/parquet_writer.py` + `query/datafusion_engine.py` | Hive `year/month/day/class/vendor` + Snappy + DataFusion prune |
| R06 | `LogForce-Perimeter-Prototype/init.sql` | `events` PG GIN `raw jsonb_path_ops` |
...
| R13 | `Branch of logforce/logforce-swarnadeep-ai-unknown-solver/unknown_solver/rules/generated/` 5× `solver_*.json` | Rule synthesis output exists |
...
| R16 | `guide/LogForce - Production Grade Project Specification.docx` + `Design Specification_ Issue #2 Pattern Mining Tier.docx` | Original guide specs (unparsed docx, cited via `LogForce-Deep-Research-Report.md`) |
...
| E04 | RethinkDNS + Orbot (Google Play) | VpnService + QS Tile pattern your screenshot shows (Refresh/Rethink/Orbot row) → copied for `LogForce` tile |
...
Each of the 7 LogForce-Docs cites local `file:line` instead of URLs so an auditor can `grep` without internet:
...
- `LogForce-Docs/AUDIT.md` is the line-cited audit of all 7 docs (scores 4.17/3.97/4.14, 6 gaps to close).
- This `References.md` is the source-of-truth index for that audit.

## 5. Principle

> **If the line isn't in `maincode/` or `LogForce-Perimeter-Prototype/`, it's a roadmap, the docs mark it as future, not done.**
