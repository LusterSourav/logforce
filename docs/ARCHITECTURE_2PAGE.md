...
Modular monolith + Vector 0.38/VRL ingestion + Go ONNX edge classifier for the **perimeter slice** (today); thin device agents + WireGuard `10.0.0.0/24` to an **Azure free VM pod** where Drain3 + `qwen2.5:0.5b` decode to fix, for the **universal fleet** (next). Constraints driving every choice: air-gapped single binary (`models/ 58M baked: onnx 215K+onnx_data 22M+2_Dense 1.5M+libonnx 34M`), budget ~5 ms warm (`server.go:79 389ms cold → ~5ms`), raw never lost (`sha256(trim(raw))` + `unmapped.raw_event`), one-regex device onboarding (`VRL:17-20 only`), one `docker compose` bring-up, `when_full=block` not drop (`vector.toml:43`).

**ONNX placement rule:** 23 MB `int8` `42 leaves ~5ms` runs **on pod/edge**, never on phone (`libonnxruntime 34M, no NNAPI` `Prd.md:67`). Phone is a thin capturer (`POST /capture ≤4096 + sha16 window200` `capture.py:11-12,24-37`).
...
```text
 ...
 │ ollama:11434 │ qwen2.5:0.5b 0.52GB (or 1b/2b/3.8b)
...
Per-app detection is one fingerprint string in `normalize_outertune.vrl:74` / `sampler.py`, no new agent.
...
**Future tile (screenshot target):** new QS row `[ Refresh Connection | ULPF ]` beside Rethink/Orbot. Kotlin `TileService:72 BIND_QUICK_SETTINGS_TILE` `onClick() → STATE_ACTIVE → IO{postToVm(logcatTail)} → toast(hint+fix) → INACTIVE` (`docs/PHONE_TILE.md:21`). `VpnService` routes only `10.0.0.0/24` (unlike Rethink `0.0.0.0/0`).

### 5. Backend, Perimeter Go + Pod Auto-Capture

**Go `ui/server.go:8081` (also on pod):**
...
**Pod `auto_capture/server.py:8002` (via WireGuard, NOT public):**
...
| `POST /parse` via `miner:8001` | `add_log_message + exact_matching` | internal |
| Fix per `hint` | e.g. `POST /api/token/refresh` | as `curl` button |

Phone never touches `127.0.0.1:11434` (`ai_engine.py:21`), sanitizer 21+9 (`sanitizer.py:51-103`) before sanitized text hits `localhost:11434` only.
...
**Data ingestion:** any device agent/crash handler → WireGuard `10.0.0.0/24` (fallback `adb reverse` + Syncthing) → `POST /capture /image /launch 4096 cap` → `logs/auto_captured.log` dedup+rotate+miner `Drain3 8001` → `Vector file → VRL OCSF` → `NDJSON 4001 + integrity.hash + unmapped.raw_event → Hive year/month/day/class/vendor Snappy → DataFusion prune` → `PG GIN + Loki:3100 + Prom:8000 → Grafana:3000 (+ /report toast)`.

**Edge engine:** WordPiece 128 → ONNX `int8` → mean-pool → projection 1024d → cosine 42 leaves threshold 0.5 → `UNCLASSIFIED` else. Cold 381ms `server.go:79`, warm ~5ms, batch 100 → 50-80ms (`SYSTEM_ARCHITECTURE.md:145`), one `Embed` call. **On VM pod + perimeter edge**, not phone.

**VM pod, free-tier + small-GB model:**

* GitHub Student Pack `education.github.com/pack → $100 + 12mo free B1s 1/1GB 750h/mo` or `B1ms 1/2GB`; static IP free while VM running, NSG `51820/udp` WG, `8002` only via `10.0.0.x` not public, outbound `443` for ephemeral runner. `az vm create -g ulpf-rg -n ulpf-pod --image Ubuntu2204 --size Standard_B1s` then `docker compose -f maincode/docker-compose.yml up -d && ollama pull qwen2.5:0.5b` (`Architecture.md:148-155`). Alternatives: Oracle `E2.1.Micro` forever, GCP `e2-micro`.
* Best small locals (disk→RAM ~1.3×, all `SOLVER_OLLAMA_MODEL` env swappable, none on phone): `qwen2.5:0.5b 0.52GB→~1GB DEFAULT B1s 0.8s/decode`, `llama3.2:1b 1.32GB→~2GB if B1ms`, `gemma2:2b 1.64GB→~3GB if B2s`, `phi3:3.8b 2.18GB→~4GB if 8GB pod`. Pipeline 10 steps `pipeline.py:145 Sampler→Sanitizer→Prompt nonce→AI→OCSF→Validator10→Tester0.8/0.1→Git→Loader` gates before commit.
...
**Project structure:** `ULPF-Perimeter-Prototype/ (models 58M + ingestion + parsing + storage + ui:8081)` + `maincode/ (vector+miner 8001+auto_capture 8002+ai_solver+storage/query+observability+prototype overlay+docs/)` + `ULPF-Docs/` 7-file safe set. `Branch of ulpf/ 99 files` 5 branches merged.

**Deployment, Day vs Night:**

Day (prototype, local): `go run./ui/server.go && open http://localhost:8081/dashboard.html` + `vector --config ingestion/vector.toml` + `python storage/parquet_writer.py --watch`.

Night (future, anywhere): `az vm create...Standard_B1s` then phone `install ULPF.apk → QS → Add Tile ULPF → tap → WireGuard → POST hint+curl → Copy → fixed` `curl -k https://10.0.0.1:8002/capture -d '{"log":"E OuterTune Source Error 2000","source":"phone"}' | jq.hint`.

**Scaling path (honest):** Siembol Storm as **slide not ship** (`SYSTEM_DESIGN.md:110`). No K8s in prototype (KISS). Ingest `file → kafka 300→1000 partitions batch10k zstd`, store `PG GIN → ClickHouse`, autoscale `KEDA on Kafka lag`.
...
| Model size | `mdbr-leaf-mt int8 42 23MB` | Hyper + `qwen2.5:0.5b 0.52GB` default (swap via env) |
| Unknown | drop | Drain3 → LLM `qwen2.5:0.5b` → validated VRL/PR + fix endpoint |
...
**Ground truth:** measured `100/60ms → ~1.6k/sec` `Prd.md:230`. `1B ÷ 1.6k = 625,000 instances`, single B1s cannot do 1B.

**Hyper (custom, not `mdbr-leaf-mt`):** teacher `bge-large` → student 4L/256d/6-head/seq64 + trimmed WordPiece 12k + fused tokenizer + dynamic int8 (CPU 11MB) / AWQ int4 (GPU 9MB) + ORT `CPU→CUDA→TensorRT` EP + hierarchical `Root8→42→400` via HNSW PQ-64 + gRPC `:50051 batch 512` bucket `32/64`. Serving: `1×A10G ~45k/s (64-len batch512 int8 ~180ms), 1×H100 ~120k/s` (`Architecture.md:259`).
...
**Claim:** `Prototype 1.6k/s. Hyper (4L/256d/int4+HNSW+GPU) 120k/s/GPU; 1B is 10-sec burst on sharded fleet (800+H100, Kafka 1000), not single VM. Sustained universal prod `100k-1M/s on 1-10 GPUs` dwarfs any SIEM.`, `Architecture.md:275-281`. Storage of 1B 10s must be sampled: `1B×1KB×86400=86 PB/day`, keep 100% counts + 1% raw (`Architecture.md:280-281`). `Ship:` distilled Hyper ONNX + GPU EP + batch512 + KEDA. `Slide:` lines/sec vs GPUs + cost/min + sampling.